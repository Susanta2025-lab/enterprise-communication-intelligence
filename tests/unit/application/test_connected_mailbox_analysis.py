...
class _RecordingConnector:
    ...
    def list_attachments(self, provider_message_id: str):
        raise AssertionError("mailbox analyze must not list attachments")

    def fetch_attachment_content(self, provider_message_id: str, provider_attachment_id: str):
        raise AssertionError("mailbox analyze must not fetch attachment content")
...
def test_refresh_failure_during_fetch_skips_ai_and_marks_reauth_required() -> None:
    unit = InMemoryUnitOfWork()
    user_id = _seed_user(unit, _principal())
    account = _seed_account(unit, user_id)
    connector = _RecordingConnector(
