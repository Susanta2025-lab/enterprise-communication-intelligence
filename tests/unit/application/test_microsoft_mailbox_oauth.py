"""Microsoft Graph mailbox OAuth application orchestration tests."""
...
_CODE = "AUTH_CODE_SENTINEL_MS_APP_111"
_REFRESH = "REFRESH_TOKEN_SENTINEL_MS_APP_222"
...
def _authorization_result(
    *,
    external_account_id: str = _EXTERNAL,
...
def test_microsoft_denial_consumes_state_without_exchange() -> None:
    service, unit, client, _store = _service()
    service.start_authorization(_principal())
    state = client.last_state
    assert state is not None
    ...
    assert (
        ...
        or unit.mailbox_authorization_session_store[stored.id].consumed_at is not None
    )
    with pytest.raises(MailboxAuthorizationSessionInvalidError):
        service.complete_authorization(code=_CODE, state=state, error=None)
    assert client.exchange_calls == 0
...
def test_successful_callback_persists_tid_oid_capabilities_and_locator() -> None:
    ...
    assert account.external_account_id == _EXTERNAL
    ...
    material = deserialize_microsoft_mailbox_secret(found.secret_material)
    assert material.refresh_token == _REFRESH
    assert material.external_account_id == _EXTERNAL
    assert b"access_token" not in found.secret_material
...
def test_partial_grant_succeeds_read_only_and_is_not_executable() -> None:
    client = FakeMailboxOAuthClient(
        _authorization_result(
    ...
    assert is_mail_send_executable(account.granted_capabilities) is False
...
def test_reauth_required_account_is_reactivated() -> None:
    user_id = uuid4()
    existing = sample_connector_account(
        user_id,
        provider="microsoft_graph",
...
def test_legacy_email_identity_is_not_merged() -> None:
    ...
    legacy = sample_connector_account(
        user_id,
        provider="microsoft_graph",
        external_account_id="mailbox@outlook.com",
    ...
    assert unit.connector_account_store[legacy.id].external_account_id == "mailbox@outlook.com"
