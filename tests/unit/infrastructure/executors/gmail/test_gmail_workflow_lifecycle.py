"""Prove Gmail executor exception types against existing workflow lifecycle handling.

This does not wire Gmail into production execution. It uses stub executors that
raise the same exception classes the Gmail adapter uses.
"""
...
def test_gmail_exception_types_are_siblings_not_failed_subtypes() -> None:
