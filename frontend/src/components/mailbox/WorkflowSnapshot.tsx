...
export function WorkflowSnapshot({ body, variant }: WorkflowSnapshotProps) {
  ...
  return (
    <section aria-labelledby={headingId} className="min-w-0">
      ...
      <pre className="mt-3 max-w-full min-w-0 overflow-x-hidden whitespace-pre-wrap break-words rounded-md border border-slate-200 bg-white p-3 font-sans text-sm text-slate-800">
        {body}
      </pre>
    </section>
  );
