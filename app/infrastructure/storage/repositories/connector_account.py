...
class SqlAlchemyConnectorAccountRepository(ConnectorAccountRepository):
    ...
    def create(self, account: NewConnectorAccount) -> ConnectorAccountRecord:
        ...
        try:
            granted = serialize_communication_capabilities(account.granted_capabilities)
    ...
    def disconnect_owned(
        ...
        status, locator, and capability metadata while remaining explicit
        about the write. Provider token revocation is outside this method.
        """
        statement = (
        ...
        *,
        ...
        statement = (
            ...
            .where(
                ConnectorAccount.id == connector_account_id,
                ConnectorAccount.user_id == user_id,
                ConnectorAccount.status.in_(
                    (
                        ConnectorAccountStatus.DISCONNECTED.value,
                        ConnectorAccountStatus.REAUTH_REQUIRED.value,
                    )
                ),
            )
            .values(**values)
...
def _to_record(row: ConnectorAccount) -> ConnectorAccountRecord:
    try:
        granted = parse_stored_communication_capabilities(row.granted_capabilities)
    except ValueError as exc:
        raise PersistenceError(_GENERIC_FAILURE) from exc
