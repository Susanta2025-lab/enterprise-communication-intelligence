...
from app.infrastructure.storage.models import ExternalIdentity, User, utc_now
...
class SqlAlchemyIdentityRepository(IdentityRepository):
    ...
    def promote_user_role_from_user_to_owner(self, user_id: UUID) -> bool:
        ...
        result = self._session.execute(
            ...
            .where(
                User.id == user_id,
                User.application_role == ApplicationRole.USER.value,
            )
            .values(
                application_role=ApplicationRole.OWNER.value,
                updated_at=utc_now(),
            )
        )
