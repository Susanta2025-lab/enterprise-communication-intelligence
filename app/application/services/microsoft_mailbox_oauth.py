"""Microsoft Graph mailbox OAuth start and callback orchestration.

Mailbox consent is separate from ECI application-user OIDC. Microsoft HTTP never
...
_PROVIDER = MailboxAuthorizationProvider.MICROSOFT_GRAPH
_PROVIDER_SLUG = _PROVIDER.value


@dataclass(frozen=True, slots=True)
class MicrosoftMailboxAuthorizationStartResult:
    """Browser redirect target for Microsoft mailbox consent."""
...
@dataclass(frozen=True, slots=True)
class MicrosoftMailboxAuthorizationCallbackResult:
    """Sanitized Microsoft connection result. Omits locators and tokens."""
...
class MicrosoftMailboxOAuthService:
    """Start Microsoft consent and complete the unauthenticated Graph callback."""
    ...
    ) -> MicrosoftMailboxAuthorizationStartResult:
        """Create a connect session and return the Microsoft authorization URL."""
    ...
    def start_reauthorization(
        ...
        """Create a reauthorize session bound to the owned Graph account."""
        return self._start_authorization(
            principal,
    ...
    def start_connect_another(
        ...
        """Create a connect-another session that requests Microsoft account selection."""
        return self._start_authorization(
            principal,
    ...
    def _start_authorization(
        ...
        *,
        ...
        return MicrosoftMailboxAuthorizationStartResult(
    ...
    def complete_authorization(
        ...
        *,
        ...
        state: str | None,
        error: str | None,
    ) -> MicrosoftMailboxAuthorizationCallbackResult:
        ...
        try:
            ...
            logger.warning(
                "microsoft_oauth_callback_exchange_failed",
                operation="callback",
                duration_ms=elapsed_ms(started_at),
                error_class=error_class(exc),
            )
        ...
        if CommunicationCapability.MAIL_READ not in exchanged.granted_capabilities:
            logger.info(
                "microsoft_oauth_required_read_grant_missing",
                operation="callback",
                duration_ms=elapsed_ms(started_at),
            )
            raise MailboxOAuthAuthorizationFailedError()
        ...
        try:
            ...
            if isinstance(exc, ServiceUnavailableError):
                raise
            logger.warning(
                "microsoft_oauth_callback_persist_failed",
                operation="callback",
                duration_ms=elapsed_ms(started_at),
                error_class=error_class(exc),
            )
        ...
        return MicrosoftMailboxAuthorizationCallbackResult(
    ...
    def _complete_reauthorization(
        ...
        *,
        ...
        try:
            ...
            raise ServiceUnavailableError(_UNAVAILABLE) from None
        logger.info(
            "microsoft_oauth_reauthorization_completed",
            operation="callback",
            connector_account_id=str(account.id),
    ...
    def _consume_provider_error(self, *, state: str | None, started_at: float) -> None:
        ...
        try:
            ...
            logger.info(
                "microsoft_oauth_provider_error_state_invalid",
                operation="callback",
                duration_ms=elapsed_ms(started_at),
            )
            raise
            ...
            raise
        logger.info(
            "microsoft_oauth_authorization_denied",
            operation="callback",
            duration_ms=elapsed_ms(started_at),
        )
        raise MailboxOAuthAuthorizationDeniedError()
    ...
    def _persist_connector_account(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "microsoft_oauth_connector_account_persistence_failed",
                operation="callback",
                error_class=error_class(exc),
            )
            raise ServiceUnavailableError(_UNAVAILABLE) from None
    ...
    def _compensate_stored_credential(self, locator: str, started_at: float) -> None:
        try:
            ...
            logger.warning(
                "microsoft_oauth_credential_cleanup_failed",
                operation="callback",
                duration_ms=elapsed_ms(started_at),
                error_class=error_class(exc),
            )
