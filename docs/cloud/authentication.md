...
Analyze and history require `communications:analyze`. Workflow proposal and approval require `communications:workflow`. Execute requires `communications:send`. Gmail mailbox connect requires `communications:connect`. These are distinct capabilities; `OIDC_REQUIRED_PERMISSION` remains the analyze permission. Least privilege: a workflow-only token cannot send mail, a send-only token cannot create or approve a workflow action, and none of those imply mailbox OAuth connect. `communications:workflow` does not authorize external sending. The Google callback is not an ECI login and does not use the API bearer token.
...
- Production mailbox OAuth authorization/refresh for Microsoft Graph
