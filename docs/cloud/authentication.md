...
6. **Mailbox credentials → Gmail / Microsoft Graph access tokens.** Opaque `ConnectorAccount.credential_ref` is resolved by `CommunicationCredentialResolver` into an on-demand `AccessTokenProvider`. Local/development and current execute composition use environment-backed token material (`ECI_COMMUNICATION_CREDENTIAL_<PROVIDER>_<NORMALIZED_REF>_ACCESS_TOKEN`). Phase 13B adds a provider-neutral `CommunicationCredentialStore` and a refreshable resolver that acquires tokens only when the callable is invoked. Secrets are not stored in PostgreSQL. This is not the ECI API JWT, not AI workload identity, and not database identity. Phase 13A adds a short-lived `MailboxAuthorizationSession` for future delegated consent: raw OAuth state is not persisted, SHA-256(state) is unique and single-use, and PKCE S256 verifiers are stored only until consume. Production Google/Microsoft OAuth, Azure Key Vault, and AWS Secrets Manager for mailbox tokens are not implemented. The refreshable resolver is not the current execute default.
...
Analyze and history require `communications:analyze`. Workflow proposal and approval require `communications:workflow`. Execute requires `communications:send`. Mailbox connect/reauthorize/disconnect will require `communications:connect`. These are distinct capabilities; `OIDC_REQUIRED_PERMISSION` remains the analyze permission. Least privilege: a workflow-only token cannot send mail, a send-only token cannot create or approve a workflow action, and none of those imply mailbox OAuth connect. `communications:workflow` does not authorize external sending. Phase 13A introduces `communications:connect` and the authorization-session security model; it does not yet expose public Google or Microsoft OAuth routes.
