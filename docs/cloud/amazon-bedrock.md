...
AWS App Runner is not used. AWS Secrets Manager is implemented as the mailbox OAuth credential store (`CREDENTIAL_STORE_BACKEND=aws_secrets_manager`); it is not used to inject `DATABASE_URL` or Bedrock keys. Phase 7 retains application stdout through CloudWatch Logs (`/ecs/eci-api-dev`, 1 day) and uses standard AWS/ECS CPU/memory metrics. Container Insights remains disabled. See [Observability](observability.md).
