...
| File | Represents |
|---|---|
| [`architecture.mmd`](architecture.mmd) | Layered system: HTTP analyze path, connector ingestion, Phase 14 mailbox list/analyze sequences in [`sequence-diagrams.md`](../architecture/sequence-diagrams.md), Phase 13 mailbox OAuth / credential-store edges, and Phase 18 attachment ports (see Phase 18 roadmap for the authoritative attachment flow). Phase 11/12 execute sequences remain in [`sequence-diagrams.md`](../architecture/sequence-diagrams.md). |
...
| [`identity.mmd`](identity.mmd) | Identity classes: Client → Entra External ID / OIDC → JWT → ECI (`users.id` / `application_role`); mailbox delegated OAuth (separate from application login); ECI → Foundry UAMI / Bedrock task role / Key Vault / Secrets Manager; GitHub → OIDC deploy identities; PostgreSQL |
| [`cicd.mmd`](cicd.mmd) | GitHub Actions quality plus PostgreSQL integration; CD build-once → ACR → Azure Container Apps and ECR → ECS |
| [`ingress.mmd`](ingress.mmd) | Azure HTTPS → Container Apps → ECI; AWS CloudFront HTTPS → HTTP ALB → ECS (Phase 16+); historical ALB-native TLS verified then torn down in Phase 8B |
...
| [`persistence-cloud.mmd`](persistence-cloud.mmd) | Same ECI application; Phase 9 CI ephemeral `postgres:16` proof; Phase 16+ colocated Azure/AWS managed PostgreSQL (see cloud docs for current stopped/scaled posture) |

## Implemented vs. Placeholder

`architecture.mmd`, `request-flow.mmd`, `mailbox-oauth.mmd`, and `provider-abstraction.mmd` describe the analyze, connector, and mailbox-OAuth paths as they exist today. `architecture.mmd` includes the Phase 10 connector path and Phase 13 credential-store edges. Bounded mailbox listing and selected-message analyze HTTP sequences live in [`sequence-diagrams.md`](../architecture/sequence-diagrams.md). Phase 18 attachment flows are documented authoritatively in the [Phase 18 roadmap](../roadmap/phase-18-secure-attachment-intelligence.md). It does not depict every workflow execute branch. Phase 11 and Phase 12 sequences live in [`sequence-diagrams.md`](../architecture/sequence-diagrams.md). Production mailbox OAuth is implemented; PostgreSQL does not store OAuth tokens. Background workers and automatic replies are not depicted because they are not implemented.
...
`identity.mmd`, `cicd.mmd`, and `ingress.mmd` describe Phase 8 identity, CI/CD, and ingress foundations. Azure HTTPS and GitHub OIDC CD are live. Phase 16D added CloudFront HTTPS in front of HTTP ALB for AWS real-bearer traffic (analyses/connector-list in that slice; later phases exercised mailbox and attachment paths). ADR-010's ALB-native TLS remains a separate historical note. Phase 19 application RBAC (`application_role`, `/me`, owner-only admin probe) is documented in [Phase 19](../roadmap/phase-19-platform-owner-identity-and-application-rbac.md) and [ADR-028](../decisions/ADR-028-platform-owner-identity-and-application-rbac.md). See [`docs/cloud/authentication.md`](../cloud/authentication.md) and [`docs/cloud/deployment.md`](../cloud/deployment.md).

`persistence.mmd` and `persistence-cloud.mmd` describe Phase 9 persistence architecture. The application is PostgreSQL-compatible. Phase 16 provisioned colocated managed Azure Flexible Server and Amazon RDS for cloud-hosted proofs (independent per cloud; no cross-cloud replication). Schema head later includes Phase 18 `18d0001` and Phase 19 `19b0001`. Treat diagram captions that still say “not provisioned” as Phase 9 historical wording. See [`docs/architecture/persistence.md`](../architecture/persistence.md) and [`docs/cloud/persistence.md`](../cloud/persistence.md).
