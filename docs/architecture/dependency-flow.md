...
Domain defines `CommunicationActionExecutor`. Application depends on that interface. Infrastructure implements the deterministic fake and the Microsoft Graph reply adapter. Application does not import those classes.
...
```text
 ...
 (WorkflowActionExecutionService) and Graph /reply adapter
```

API does not import executor implementations. There is no executor factory and no HTTP execute route. Production workflow composition still injects the fake. The Graph writer is available for later account-driven routing.
...
- **Mailbox HTTP stays in infrastructure connectors and write adapters.** Gmail and Microsoft Graph read adapters use `httpx` REST. The Graph reply writer uses the same `httpx` approach under `app/infrastructure/executors/`. They do not add a Gmail SDK, Microsoft Graph SDK, or MSAL dependency inside ECI.
