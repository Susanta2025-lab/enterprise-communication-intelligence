...
- **Outer implementations are not storage-only:** persistence adapters live in `app/infrastructure/storage`, AI provider adapters in `app/providers`, communication connector adapters in `app/infrastructure/connectors`, mailbox credential resolution in `app/infrastructure/credentials`, and the Phase 11D fake executor in `app/infrastructure/executors`. Outer layers implement ports defined inward.
