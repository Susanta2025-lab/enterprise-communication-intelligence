...
- `app/application` depends on `app/domain` (models, schemas, AI and persistence interfaces) and `app/core` (logging, exceptions).
...
- `app/domain` depends on nothing else in the application — it is the innermost, most stable layer. Persistence contracts live there as interfaces, not as ORM models.
...
- **Not full DDD:** ECI Platform borrows clean architecture's *layering and dependency direction*, but does not implement full Domain-Driven Design — there are no aggregates, domain events, or a ubiquitous-language modeling process. Phase 9 adds repository and unit-of-work interfaces for persistence only.
- **Application services remain few and focused:** `CommunicationAnalysisService` stays AI-only. Workflow, identity, and history services orchestrate persistence around it. No generic use-case base class or command/query separation has been introduced.
- **Framework coupling still exists at the edges by design:** `app/api` necessarily depends on FastAPI (`Depends`, `APIRouter`), `app/providers/microsoft_foundry` depends on Azure SDKs, `app/providers/amazon_bedrock` depends on boto3, and `app/infrastructure/storage` depends on SQLAlchemy. This is intentional — clean architecture pushes volatility to the edges rather than eliminating it.
