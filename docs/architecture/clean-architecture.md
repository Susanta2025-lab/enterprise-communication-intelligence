...
- **Framework coupling still exists at the edges by design:** `app/api` necessarily depends on FastAPI (`Depends`, `APIRouter`), `app/providers/microsoft_foundry` depends on Azure SDKs, and `app/providers/amazon_bedrock` depends on boto3. This is intentional — clean architecture pushes volatility to the edges rather than eliminating it.
