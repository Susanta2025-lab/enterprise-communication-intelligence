...
- `app/infrastructure/executors` depends on `app/domain` (`CommunicationActionExecutor`) and `app/core` (`CommunicationActionExecutionError`). Phase 11D implements a deterministic fake only. It does not import API, SQLAlchemy, connectors, or `AIProvider`.
- `app/domain` depends on nothing else in the application — it is the innermost, most stable layer. Persistence, connector, and executor contracts live there as interfaces, not as ORM models or vendor SDKs.
...
- It defines enums, Pydantic models (including `WorkflowAction`), schemas, the `AIProvider` interface, the `CommunicationConnector` contract, and the `CommunicationActionExecutor` write port.
...
- **Action executor adapters** are confined to `app/infrastructure/executors/`. Application execution depends on `CommunicationActionExecutor`, not the fake class. There is no executor factory in Phase 11D.
...
- **Executor replaceability:** Because `WorkflowActionExecutionService` only knows about `CommunicationActionExecutor`, a later Gmail or Graph write adapter can replace the fake without changing proposal/approval HTTP or `CommunicationConnector`.
...
- **Outer implementations are not storage-only:** persistence adapters live in `app/infrastructure/storage`, AI provider adapters in `app/providers`, communication connector adapters in `app/infrastructure/connectors`, and the Phase 11D fake executor in `app/infrastructure/executors`. Outer layers implement ports defined inward.
