...
- `app/infrastructure/credentials` depends on `app/domain` (`CommunicationCredentialResolver`, `CommunicationCredentialStore`) and `app/core` (credential-neutral exceptions and logging). It implements environment-backed lookup, an in-memory credential store, locator issuance, and a refreshable resolver that performs secret/token work only when the returned callable is invoked. It does not import API, SQLAlchemy, Gmail/Graph adapters, Azure Key Vault, AWS Secrets Manager, or OAuth SDKs.
...
- **Credential replaceability:** Because write adapters depend on `CommunicationCredentialResolver` rather than `os.environ` or a cloud secret SDK, the environment-backed local resolver and the Phase 13B refreshable resolver share the same port. Key Vault and Secrets Manager remain future `CommunicationCredentialStore` implementations.
