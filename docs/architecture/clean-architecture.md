...
- `app/infrastructure/executors` depends on `app/domain` (`CommunicationActionExecutor`) and `app/core` (execution and unavailability errors). It implements a deterministic fake and a Microsoft Graph `/reply` adapter. It does not import API, SQLAlchemy, connector adapters, or `AIProvider`.
...
- **Action executor adapters** are confined to `app/infrastructure/executors/`. Application execution depends on `CommunicationActionExecutor`, not a concrete fake or Graph class. There is no executor factory. The Graph writer is not injected in 12C.
...
- **Executor replaceability:** Because `WorkflowActionExecutionService` only knows about `CommunicationActionExecutor`, a Graph write adapter can replace the fake later without changing proposal/approval HTTP or `CommunicationConnector`. 12C implements that adapter without wiring it into the service.
