...
This directory documents the architecture implemented through **Phase 11B – Workflow Persistence & User Ownership**. Provider adapters, cloud hosting, portable structured telemetry, application-user OIDC, GitHub OIDC CI/CD, user-owned PostgreSQL persistence, vendor-neutral communication connectors, the approval-gated workflow domain, and durable user-owned workflow actions are implemented. Workflow HTTP, execution, managed cloud databases, production mailbox OAuth, and send/reply are not implemented.
