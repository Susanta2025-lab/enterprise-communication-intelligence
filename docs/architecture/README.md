...
This directory documents the architecture implemented through **Phase 12C – Microsoft Graph Reply Executor** (in progress). Provider adapters, cloud hosting, portable structured telemetry, application-user OIDC, GitHub OIDC CI/CD, user-owned PostgreSQL persistence, vendor-neutral communication connectors, the approval-gated workflow domain, durable user-owned workflow actions, the workflow HTTP surface, a below-HTTP execution boundary, execution-target provenance, a provider-neutral credential-resolution port, and a standalone Microsoft Graph reply adapter are implemented. The Graph writer is not routed into workflow execution or HTTP. Gmail writes, an HTTP execute route, managed cloud databases, and production mailbox OAuth are not implemented.
