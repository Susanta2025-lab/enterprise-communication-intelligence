...
These diagrams describe the request flows implemented as of Phase 5. Source `.mmd` files live in [`docs/diagrams/`](../diagrams/README.md); the successful and failure communication-analysis flows are combined in [`request-flow.mmd`](../diagrams/request-flow.mmd). This page also documents the health request flow, which has no dedicated `.mmd` file since it involves no failure branching worth diagramming separately.
