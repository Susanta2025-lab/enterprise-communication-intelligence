...
This directory documents the REST API implemented through completed Phase 13 of the ECI Platform roadmap, plus the Phase 14A authorization and public mailbox contract (routes not served yet). Phase 10 added no connector message-ingestion HTTP endpoints. Phase 11C adds workflow proposal and approval routes. Phase 12E adds `POST /api/v1/workflow-actions/{action_id}/execute` protected by `communications:send`. Phase 13C/13D add Gmail and Microsoft mailbox authorize (`communications:connect`) and unauthenticated provider callbacks. Phase 13F adds owned disconnect and reauthorize. Phase 14A adds `communications:read` and freezes the mailbox list/analyze schemas without mounting those routes. There is no retry route and no connector fetch/analyze HTTP yet.
