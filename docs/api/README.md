...
This directory documents the REST API implemented through Phase 13C of the ECI Platform roadmap. Phase 10 added no connector HTTP endpoints. Phase 11C adds workflow proposal and approval routes. Phase 12E adds `POST /api/v1/workflow-actions/{action_id}/execute` protected by `communications:send`. Phase 13C adds Gmail mailbox authorize (`communications:connect`) and the unauthenticated Google callback. There is no retry route.
