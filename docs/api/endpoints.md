...
- **Status codes:** `401` unauthenticated; `403` missing `communications:read`; `404` unknown or not-owned connector account; `409` owned account not currently usable, including after confirmed permanent refresh failure that persisted `REAUTH_REQUIRED`; `400` invalid/expired pagination cursor when the connector identifies that condition; `422` invalid query; `503` transient credential, refresh, provider, or mailbox-HTTP unavailability (including 401/403 after a valid token). Repeated list requests are independent reads.
- **Behavior:** ownership and mailbox usability are established before credential I/O or mailbox HTTP. Confirmed permanent OAuth refresh failure marks the exact owned `ACTIVE` account `REAUTH_REQUIRED` without deleting the credential, clearing grants, or disconnecting. Transient failures do not mutate lifecycle. Mailbox HTTP 401/403 after a valid token is not treated as `invalid_grant`. One list request corresponds to one bounded connector page. Messages are not persisted. AI is not invoked. No workflow action is created and no send/reply occurs.
