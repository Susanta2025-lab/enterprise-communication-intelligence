...
Phase 14 — Connected Mailbox Read and Analysis is Next. Architecture is reconciled and not yet implemented: `communications:read` plus `communications:analyze` for mailbox analyze, bounded provider-neutral message listing, and a provider-neutral read connector factory. Live IdP scope provisioning for `communications:read` is deferred until controlled Phase 14 live validation. Details: [Phase 14](phase-14-connected-mailbox-analysis.md).
