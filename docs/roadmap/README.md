...
| Phase 14 – Connected Mailbox Read and Analysis | Next |
...
Phase 14 — Connected Mailbox Read and Analysis is Next. **14A is Completed** (`communications:read`, public list/analyze contract, mailbox-read error types, ADR-024). **14B is Completed** (provider-neutral `CommunicationConnectorFactory`, Gmail/Graph routing, lazy access tokens, read-side DI independent of `communications:send`). **14C is Completed** (mounted mailbox-backed analyze for an owned ACTIVE/read-capable account; direct-text analyze unchanged). **14D is Completed** (bounded mailbox listing through `GET /api/v1/connector-accounts/{id}/messages`, `communications:read`, opaque cursor, Graph `nextLink` not exposed). **14E is Completed** (confirmed permanent OAuth refresh failure persists exact-owned `ACTIVE → REAUTH_REQUIRED` on list and analyze; transient failures and mailbox HTTP 401/403 do not mutate lifecycle; privacy sentinels stay out of responses and logs). Remaining slice: live validation plus README audit. Live IdP scope provisioning for `communications:read` is deferred until controlled Phase 14 live validation. Details: [Phase 14](phase-14-connected-mailbox-analysis.md).
