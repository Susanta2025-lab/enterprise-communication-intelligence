...
Phase 17A — External ID Readiness Assessment is **Completed / PASS**. Phase 17B-A — ADR / Architecture Lock is **Completed / PASS**: [ADR-027](../decisions/ADR-027-microsoft-entra-external-id-customer-authentication.md) locks Microsoft Entra External ID as the exclusive ECI product-login IdP, email OTP as the initial customer path, retained `(iss, sub)` mapping, no dual issuer, no schema migration, and mailbox-login separation. Phase 17B-B — Frontend External ID / MSAL Configuration is **Completed / PASS**: explicit CIAM `VITE_ENTRA_AUTHORITY` and derived `knownAuthorities`; existing MSAL SPA retained. Phase 17B-C — Backend External ID JWT / Configuration is **Completed / PASS**: existing single-issuer `TokenValidator` retained; CIAM-shaped offline JWT tests and External ID OIDC placeholders. Next is 17B-D offline authentication regression. 17C controlled owner-account validation and 17D Sally verification remain later. Details: [Phase 17](phase-17-external-id-external-user-onboarding.md).
