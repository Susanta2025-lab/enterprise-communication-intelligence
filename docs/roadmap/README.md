...
| Phase 16 – Cloud-Hosted Browser & End-to-End Multi-Cloud Validation | 16A–16B completed; 16C complete — ready for commit; 16D–16F not started |
...
Phase 16A — Cloud Runtime / Deployment Readiness is completed: authenticated read-only Azure and AWS inventory, ADR-026 topology freeze (Azure Static Web Apps → ACA; AWS S3/CloudFront SPA and CloudFront → HTTP ALB → ECS without a custom domain), colocated sequential PostgreSQL, mandatory Key Vault / Secrets Manager, configuration and cost/authorization gates. No cloud resources were created or mutated in 16A. Phase 16B — Azure Full-Stack Browser Deployment is completed: Azure Static Web Apps `eci-web-dev`, Azure Database for PostgreSQL Flexible Server `eci-pg-dev-susanta`, current-master ACA image `eci-api:7518360`, Key Vault Secrets Officer on the runtime UAMI, Entra/MSAL browser smoke against ACA. Phase 16C — Azure Live Mailbox → Microsoft Foundry Validation is complete and ready for commit: Graph delegated OAuth, Key Vault credential durability across an ACA same-revision recycle, one `MicrosoftFoundryProvider` selected-message analysis, explicit Propose (PENDING) then explicit Approve (APPROVED), stop before Send. No Gmail, Bedrock, AWS, execute, or live Send. No application-code or schema change. 16D–16F are not started. Details: [Phase 16](phase-16-cloud-browser-multicloud-validation.md).
