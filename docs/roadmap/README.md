...
| Phase 16 – Cloud-Hosted Browser & End-to-End Multi-Cloud Validation | 16A–16D completed; 16E next; 16F-A1 completed; remaining 16F not started |
...
Phase 16A — Cloud Runtime / Deployment Readiness is completed: authenticated read-only Azure and AWS inventory, ADR-026 topology freeze (Azure Static Web Apps → ACA; AWS S3/CloudFront SPA and CloudFront → HTTP ALB → ECS without a custom domain), colocated sequential PostgreSQL, mandatory Key Vault / Secrets Manager, configuration and cost/authorization gates. No cloud resources were created or mutated in 16A. Phase 16B — Azure Full-Stack Browser Deployment is completed: Azure Static Web Apps `eci-web-dev`, Azure Database for PostgreSQL Flexible Server `eci-pg-dev-susanta`, current-master ACA image `eci-api:7518360`, Key Vault Secrets Officer on the runtime UAMI, Entra/MSAL browser smoke against ACA. Phase 16C — Azure Live Mailbox → Microsoft Foundry Validation is completed: Graph delegated OAuth, Key Vault credential durability across an ACA same-revision recycle, one `MicrosoftFoundryProvider` selected-message analysis, explicit Propose (PENDING) then explicit Approve (APPROVED), stop before Send. No Gmail, Bedrock, AWS, execute, or live Send. Phase 16D — AWS HTTPS + Full-Stack Browser Deployment is **COMPLETE / PASS**: private S3 + CloudFront OAC SPA `https://d1ut7j94w7lt3b.cloudfront.net`, API CloudFront `https://dnookm0ucbhv1.cloudfront.net` → HTTP ALB → ECS `eci-api-dev:6` image `0050b30`, RDS `eci-pg-dev`, Secrets Manager credential-store backend, Entra/MSAL, CORS, protected analyses and connector-list reads. Connector-list 503 resolved by decoupling listing from the OAuth store. No Gmail, Graph mailbox, Bedrock inference, or Send. Phase 16E — AWS Gmail → Bedrock validation is **Next** (not started). Phase 16F-A1 — Connector UX Redesign is **Completed**: Connected Mailboxes cards now show account identity, **Reconnect same account**, and a truthful provider-level connect-another affordance that is not wired to current OAuth. Remaining 16F remains later parity, security/cost hardening, temporary IAM cleanup, and final documentation. Details: [Phase 16](phase-16-cloud-browser-multicloud-validation.md).
